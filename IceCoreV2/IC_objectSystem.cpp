...
void IC_objectSystem::FinishSpawn(int id, bool persistent)
{
	if (persistent)
	{
		if (persistentObjects.contains(id))
		{
			const auto& object = persistentObjects[id];
			if(object->inPlay) 
			{ 
				game->ICLog("Warning: Persistent object with id " + std::to_string(id) + " is already in play. BeginPlay was not called.");
				return; 
			}
			object->inPlay = true;
			object->BeginPlay();
		...
		{
			game->ICLog("Warning: Object with id " + std::to_string(id) + " does not exist.");
			return;
		}
	}
	else
	{
		if (objects.contains(id))
		{
			...
			if (object->inPlay)
			{
				game->ICLog("Warning: Object with id " + std::to_string(id) + " is already in play. BeginPlay was not called.");
				return;
			}
			object->inPlay = true;
			object->BeginPlay();
...
void IC_objectSystem::RequestDestroy(IC_object* object)
{
	...
	if (id == -1)
	{
		game->ICLog("Warning: Attempted to destroy an object with no id.");
		return;
	}
